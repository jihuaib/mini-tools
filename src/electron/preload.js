...
contextBridge.exposeInMainWorld("stringGeneratorApi", {
    generateTemplateString: (templateData) =>
        ipcRenderer.invoke("generate-template-string", templateData),
    saveStringGeneratorConfig: (config) =>
        ipcRenderer.invoke("save-string-generator-config", config),
    loadStringGeneratorConfig: () =>
        ipcRenderer.invoke("load-string-generator-config"),
});
...
contextBridge.exposeInMainWorld("bgpEmulatorApi", {
    ...
    updateBgpData: (callback) =>
        ipcRenderer.on("update-bgp-data", (_event, data) => callback(data)),
