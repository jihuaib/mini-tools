<template>
    ...
    >
        ...
        <a-row>
            ...
            <a-col :span="12">
                <a-form-item label="Local AS" name="localAs">
                    <a-tooltip
                        :title="validationErrors.localAs"
                        :open="!!validationErrors.localAs"
                    >
                        <a-input
                            v-model:value="bgpData.localAs"
                            @blur="validateLocalAs"
        ...
        <a-row>
            ...
            <a-col :span="12">
                <a-form-item label="Peer AS" name="peerAs">
                    <a-tooltip
                        :title="validationErrors.peerAs"
                        :open="!!validationErrors.peerAs"
                    >
                        <a-input
        ...
        <a-row>
            <a-col :span="12">
                <a-form-item label="Router ID" name="routerId">
                    <a-tooltip
                        :title="validationErrors.routerId"
                        :open="!!validationErrors.routerId"
                    >
                        <a-input
                            v-model:value="bgpData.routerId"
                            @blur="validateRouterId"
            ...
            <a-col :span="12">
                <a-form-item label="Hold Time" name="holdTime">
                    <a-tooltip
                        :title="validationErrors.holdTime"
                        :open="!!validationErrors.holdTime"
                    >
                        <a-input
                            v-model:value="bgpData.holdTime"
                            @blur="validateHoldTime"
        ...
        <a-form-item label="Open Cap" name="openCap">
            <a-space>
                <a-checkbox-group
                    v-model:value="bgpData.openCap"
                    :options="openCapOptions"
                />
                <a-button
                    type="link"
                    @click="showCustomOpenCap"
                    class="custom-route-btn"
                >
            ...
            <a-row>
                <a-col :span="8">
                    <a-form-item label="Prefix" name="ipv4RouteConfig.prefix">
                        <a-tooltip
                            :title="validationErrors.ipv4Prefix"
                            :open="!!validationErrors.ipv4Prefix"
                        >
                            <a-input
                                v-model:value="bgpData.ipv4RouteConfig.prefix"
                                @blur="validateIpv4Prefix"
                                :status="
                                    validationErrors.ipv4Prefix ? 'error' : ''
                                "
                            />
                        </a-tooltip>
                    </a-form-item>
                ...
                <a-col :span="8">
                    <a-form-item label="Mask" name="ipv4RouteConfig.mask">
                        <a-tooltip
                            :title="validationErrors.ipv4Mask"
                            :open="!!validationErrors.ipv4Mask"
                        >
                            <a-input
                                v-model:value="bgpData.ipv4RouteConfig.mask"
                                @blur="validateIpv4Mask"
                                :status="
                                    validationErrors.ipv4Mask ? 'error' : ''
                                "
                            />
                        </a-tooltip>
                    </a-form-item>
                ...
                <a-col :span="8">
                    <a-form-item label="Count" name="ipv4RouteConfig.count">
                        <a-tooltip
                            :title="validationErrors.ipv4Count"
                            :open="!!validationErrors.ipv4Count"
                        >
                            <a-input
                                v-model:value="bgpData.ipv4RouteConfig.count"
                                @blur="validateIpv4Count"
                                :status="
                                    validationErrors.ipv4Count ? 'error' : ''
                                "
                            />
                        </a-tooltip>
                    </a-form-item>
            ...
            <a-row>
                <a-col :span="8">
                    <a-form-item label="Prefix" name="ipv6RouteConfig.prefix">
                        <a-tooltip
                            :title="validationErrors.ipv6Prefix"
                            :open="!!validationErrors.ipv6Prefix"
                        >
                            <a-input
                                v-model:value="bgpData.ipv6RouteConfig.prefix"
                                @blur="validateIpv6Prefix"
                                :status="
                                    validationErrors.ipv6Prefix ? 'error' : ''
                                "
                            />
                        </a-tooltip>
                    </a-form-item>
                ...
                <a-col :span="8">
                    <a-form-item label="Mask" name="ipv6RouteConfig.mask">
                        <a-tooltip
                            :title="validationErrors.ipv6Mask"
                            :open="!!validationErrors.ipv6Mask"
                        >
                            <a-input
                                v-model:value="bgpData.ipv6RouteConfig.mask"
                                @blur="validateIpv6Mask"
                                :status="
                                    validationErrors.ipv6Mask ? 'error' : ''
                                "
                            />
                        </a-tooltip>
                    </a-form-item>
                ...
                <a-col :span="8">
                    <a-form-item label="Count" name="ipv6RouteConfig.count">
                        <a-tooltip
                            :title="validationErrors.ipv6Count"
                            :open="!!validationErrors.ipv6Count"
                        >
                            <a-input
                                v-model:value="bgpData.ipv6RouteConfig.count"
                                @blur="validateIpv6Count"
                                :status="
                                    validationErrors.ipv6Count ? 'error' : ''
                                "
                            />
                        </a-tooltip>
                    </a-form-item>
        ...
        <a-form-item :wrapper-col="{ offset: 10, span: 20 }">
            <a-space>
                ...
                <a-button type="primary" danger @click="withdrawRoutes"
                    >撤销路由</a-button
                >
            </a-space>
        </a-form-item>
...
onMounted(async () => {
    ...
    if (savedConfig.status === "success" && savedConfig.data) {
        ...
        bgpData.value.openCap = Array.isArray(savedConfig.data.openCap)
            ? [...savedConfig.data.openCap]
            : [];
        bgpData.value.addressFamily = Array.isArray(
        ...
        if (savedConfig.data.routeConfig) {
            bgpData.value.routeConfig.ipType =
                savedConfig.data.routeConfig.ipType || "ipv4";
        }
    }
});
...
const validateIpv4Mask = () => {
    if (!bgpData.value.ipv4RouteConfig.mask) {
        ...
        !/^(?:[0-9]|[1-2][0-9]|3[0-2])$/.test(
            bgpData.value.ipv4RouteConfig.mask,
        )
    ) {
...
const validateIpv6Mask = () => {
    if (!bgpData.value.ipv6RouteConfig.mask) {
        ...
        !/^(?:[0-9]|[1-9][0-9]|1[0-2][0-8])$/.test(
            bgpData.value.ipv6RouteConfig.mask,
        )
    ) {
...
const sendRoutes = async () => {
    try {
        const currentConfig =
            bgpData.value.routeConfig.ipType === "ipv4"
                ? bgpData.value.ipv4RouteConfig
                : bgpData.value.ipv6RouteConfig;
